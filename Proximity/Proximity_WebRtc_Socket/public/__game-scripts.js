...
PlayerMovement.prototype.update = function (dt) {
    ...
    if (selectedElem == "CANVAS") {
        ...
        if (app.keyboard.isPressed(pc.KEY_E)) {
            this.app.root.findByName("Brunnen").sound.play("geil");
        }
        ...
        if (app.keyboard.isPressed(pc.KEY_ESCAPE)) {
            this.app.root.findByName("2D Screen").enabled = true;
        }
    ...
    if (x !== 0 || z !== 0) {
        ...
        if (this.running.state === true) {
            force.set(x, 0, z).normalize().scale(this.movementSpeed * this.movementMultiplier * 0.5);
        } else {
...
PlayerMovement.attributes.add('serverAddress', {
    type: 'text',
    default: ""
});
...
Network.prototype.movePlayer = function (data) {
    if (this.initialized && !this.players[data.id].deleted) {
        ...
        if (this.players[data.id].entity.children[0].animation.data.currAnim != data.currAnim) {
            this.players[data.id].entity.children[0].animation.play(data.currAnim, 0.25);
        }
...
Network.prototype.createPlayerEntity = function (data) {
    ...
    if (data) {
        ...
        newPlayer.setEulerAngles(new pc.Vec3(data.rotx, data.roty, data.rotz));
        console.log(data);
    }
...
Network.prototype.updatePosition = function () {
    if (this.initialized) {
        ...
        // TODO only emit if data has changed
        Network.socket.emit('positionUpdate', {
            ...
            roty: rot.y,
            rotz: rot.z,
            currAnim: this.app.root.findByName("Model").animation.currAnim,
        });
    }
};
...
// nameScript.js
var NameScript = pc.createScript('nameScript');

// initialize code called once per entity
NameScript.prototype.initialize = function () {
    camera = this.app.root.findByName("Camera");
};

// update code called every frame
NameScript.prototype.update = function (dt) {
    this.entity.setRotation(camera.getRotation());
};

// swap method called for script hot-reloading
// inherit your script state here
// NameScript.prototype.swap = function(old) { };
...
// update code called every frame
Respawn.prototype.update = function (dt) {
    //console.log(this.entity);
    if (this.entity.getLocalPosition().y < -0.2) {
        this.entity.sound.play("scream");
        if (this.entity.getLocalPosition().y < -1.5) {
...
// swap method called for script hot-reloading
// inherit your script state here
// Respawn.prototype.swap = function(old) { };

// to learn more about script anatomy, please read:
// https://developer.playcanvas.com/en/user-manual/scripting/

// video.js
var Video = pc.createScript('video');
...
Video.attributes.add('playEvent', {
    title: 'Play Event',
...
Video.prototype.initialize = function () {
    ...
    socket.on("setUpVideos", data => {
        videosId = data;
        videosId.forEach(element => {
            videos[element] = document.createElement('video')
            videos[element].setAttribute("id", element);
        });
        Object.keys(videos).map(function(k) {
            document.body.appendChild(videos[k]);
         });

        console.log(videosId)
    })
    var app = this.app;
    //Use socket to define id for video elements
    //Socket eine zahl mitgeben und nach dem ersten mal aufh�ren direkt
    socket.on('initializeVideo', (socket_id, initialVideoIds) => {
        
        //Only initialize the first video entity
        //The next one will be handled by web rtc. Multiple functionality alrdy implemented but not used
        if(this.entity._parent.name != "OtherVideo"){
            
        console.log("I am " + socket_id)
        initialVideoIds.forEach(element => {
            videos[element] = document.createElement('video')
            videos[element].setAttribute("id", element);
        });
        ...
        navigator.mediaDevices.getUserMedia(constraints).then(stream => {
            ...
            videos[socket_id].srcObject = stream;
            localStream = stream;


            videos[socket_id].autoplay = true;
            videos[socket_id].crossOrigin = 'anonymous';
            videos[socket_id].loop = true;
            // muted attribute is required for videos to autoplay
            //video.muted = true;

            // critical for iOS or the video won't initially play, and will go fullscreen when playing
            videos[socket_id].playsInline = true;
            ...
            var style = videos[socket_id].style;
            style.width = '1px';
            style.height = '1px';
            ...
            this.videoTexture.setSource(videos[socket_id]);
  
            videos[socket_id].addEventListener('canplay', function (e) {
        ...
        socket.on('initReceive', socket_id => {
            ...
            this.addPeer(socket_id, false, app)
            socket.emit('initSend', socket_id)
        })
        
        socket.on('initSend', socket_id => {
            console.log('INIT SEND ' + socket_id)
            this.addPeer(socket_id, true, app)
        })
...
/*
  ...
  */
  Video.prototype.openPictureMode = function (el) {
 ...
 */
  Video.prototype.removePeer = function (socket_id) {
 ...
 */
  Video.prototype.addPeer = function (socket_id, am_initiator, app) {
...
*/
     ...
     Object.keys(videos).map(function(k) {
        ...
        // Create a texture to hold the video frame data            
        data.videoTexture = new pc.Texture(app.graphicsDevice, {
            format: pc.PIXELFORMAT_R8_G8_B8,
            minFilter: pc.FILTER_LINEAR_MIPMAP_LINEAR,
            magFilter: pc.FILTER_LINEAR,
            addressU: pc.ADDRESS_CLAMP_TO_EDGE,
        ...
        data.videoTexture.setSource(videos[socket_id]);
        console.log(videos)
      
        videos[socket_id].addEventListener('canplay', function (e) {
            console.log(e)
            data.app.fire(data.playEvent, data.videoTexture);
        }.bind(data));
...
// swap method called for script hot-reloading
// inherit your script state here
// Video.prototype.swap = function(old) { };
...
Screen.attributes.add('screenMaterial', {
...
Screen.attributes.add('playEvent', {
    title: 'Play Event',
...
/*
// screenOther.js
var Screen = pc.createScript('screenOther');

Screen.attributes.add('screenMaterial', {
    title: 'Screen Material Other',
    description: 'The screen material of the TV that displays the video texture.',
    type: 'asset',
    assetType: 'material'
});
...
// socket.js
// SOCKET.IO CLIENT
// Version 2.0.4 from https://cdnjs.com/libraries/socket.io
